...
export const StoreHero: React.FC<StoreHeroProps> = ({
  ...
  const defaultSlides = [
    {
      id: '',
      badge: '100% Bahan Alami & Terjamin',
      title: shopInfo?.business_name ? `Solusi Sehat Alami di ${shopInfo.business_name}` : 'Solusi Sehat Alami Untuk Keluarga',
      description: shopInfo?.description || 'Produk herbal berkualitas pilihan alam terbaik untuk mendukung kesehatan dan vitalitas tubuh setiap hari.',
      price: null,
    ...
    {
      id: '',
      badge: 'Formula Tradisional Asli',
      title: 'Kekayaan Rempah & Herbal Pilihan',
      description: 'Diracik secara higienis dengan standar mutu tinggi untuk khasiat maksimal dan aman dikonsumsi rutin.',
      price: null,
  ...
  const slides = productSlides.length > 0 
    ? productSlides.map(p => ({
        ...
        title: p.name,
        description: p.description || 'Pilihan alami terbaik dengan bahan herbal berkhasiat untuk menjaga kebugaran tubuh setiap hari.',
        price: p.price,
          ...
          <div className="md:col-span-6 space-y-4 text-left">
            ...
            {current.price !== null && (
              <div className="flex items-baseline gap-2 pt-1">
                ...
                <span className="text-xs text-muted-foreground font-medium">/ botol / pack</span>
              </div>
            )}
