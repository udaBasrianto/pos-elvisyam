...
import { 
  ...
  Heart, ShieldCheck, Truck, MessageCircle, 
  Check, Leaf, ChevronRight, RefreshCw 
} from 'lucide-react';
...
export const StoreProductDetail = () => {
  ...
  const loadShopInfo = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/store/info/${activeIdentifier}`);
      if (response.ok) {
  ...
  const handleWhatsAppOrder = () => {
    if (!product) return;
    const phone = (shopInfo?.business_phone || shopInfo?.whatsapp_number || '').replace(/[^0-9]/g, '');
  ...
  return (
    ...
    >
      {/* 🚀 Dynamic Rich OpenGraph & Google Product Schema (JSON-LD) */}
      <StoreSeoHead
        title={`${product.name} - Rp ${product.price?.toLocaleString('id-ID')}`}
        description={product.description || `Beli ${product.name} dengan harga Rp ${product.price?.toLocaleString('id-ID')}. Kualitas produk terjamin dan pengiriman cepat.`}
        image={product.image || undefined}
        type="product"
        storeName={shopInfo?.business_name || 'Toko Online'}
        faviconUrl={shopInfo?.favicon_url || shopInfo?.logo_url || shopInfo?.business_logo}
        productData={{
          id: product.id,
          name: product.name,
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <Button 
              ...
              className="rounded-full h-9 w-9 text-foreground hover:bg-muted shrink-0 cursor-pointer"
              aria-label="Kembali"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={`rounded-full h-9 w-9 transition-colors cursor-pointer ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 pt-1">
              <div className="p-3 rounded-2xl bg-card border border-border/60 flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-bold text-foreground">Asli & Terjamin</p>
                  <p className="text-[10px] text-muted-foreground truncate">Mutu Terstandar</p>
                </div>
              </div>
              ...
              <div className="p-3 rounded-2xl bg-card border border-border/60 col-span-2 sm:col-span-1 flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-bold text-foreground">Layanan CS</p>
                  <p className="text-[10px] text-muted-foreground truncate">Bantuan Cepat</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 space-y-6 text-left">
              ...
              <div className="flex items-center gap-4">
                <div className="inline-flex items-center border border-border/80 rounded-full p-1 bg-card shadow-2xs">
                  <Button
                    ...
                    className="h-9 w-9 rounded-full text-foreground hover:bg-muted cursor-pointer"
                  >
            ...
            <div className="space-y-3 pt-4 border-t border-border/60">
              <h2 className="text-base font-extrabold text-foreground">Deskripsi Produk</h2>
              <div className="text-xs sm:text-sm text-muted-foreground leading-relaxed whitespace-pre-line space-y-2">
                {product.description || (
                  <p className="italic text-muted-foreground/80">
                    Produk berkualitas tinggi yang siap dikirim dengan jaminan keamanan dan kepuasan pelanggan.
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="hidden sm:block">
        <StoreBottomNav themeColor={shopInfo?.theme_color} />
      </div>
    </div>
  );
};
